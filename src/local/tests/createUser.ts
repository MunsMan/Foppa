...
export const getFunctions = async (token: AuthToken) => {
    return axios.get<UserFunctionsResponse>(`${SERVICE_URL}/function/${token.username}`,{headers: {'Authorization': token.token}})
}

export const deleteUser = async (token: AuthToken) => {
    return axios.delete(`${SERVICE_URL}/user/${token.username}`,{headers: {'Authorization': token.token }})
}
...
const main = async () => {
    for(let i = 0; i < 5; i++){
        ...
        const warmStart = await Promise.all(Array(10).fill(1).map((_, index)=>iteration(`warmStart-${index}`)))
